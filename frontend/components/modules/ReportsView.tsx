            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stats?.last7 ?? []} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="total" name="Leads" radius={[4, 4, 0, 0]}>
                  {(stats?.last7 ?? []).map((_: { day: string; date: string; total: number }, i: number, arr: { day: string; date: string; total: number }[]) => (
          ...
          {isLoading ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                ...
                >
                  {(stats?.byStatus ?? []).map((s: { name: string; color: string; total: number }, i: number) => (
        ...
        <Section title="Branch Breakdown">
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {(stats?.branchBreakdown ?? []).map((b: { branch: string; leads: number; converted: number; conversion_rate: number }, i: number) => (
                  <tr key={i}>
                    <Td>{b.branch}</Td>
                    <Td>{b.leads}</Td>
                    <Td>{b.converted}</Td>
...
function TeamTab({ branchId, branches, isOwnerOrManager }: {
  branchId: string; branches: ReportBranch[]; isOwnerOrManager: boolean
}) {
...
function SourcesTab({ branchId, branches }: { branchId: string; branches: ReportBranch[] }) {
  const [filters, setFilters] = useState<Pick<ReportFilters, 'branch_id' | 'date_from' | 'date_to'>>({})
  const activeFilters = { ...filters, ...(branchId ? { branch_id: branchId } : {}) }
...
export default function ReportsView() {
  ...
  const { data: branches  } = useReportBranches()    as { data: ReportBranch[]    | undefined }
  const { data: statuses  } = useReportStatuses()    as { data: ReportStatus[]    | undefined }
  const { data: teamMembers } = useReportTeamMembers() as { data: ReportTeamMember[] | undefined }
      ...
      <div style={{
        ...
        {showBranchSelector && (
          ...
          >
            ...
            {safeB.map((b: ReportBranch) => <option key={b.id} value={b.id}>{b.name}</option>)}
          </select>
        )}
      </div>
